...
`fast-boot.php` reads `config/platform.php` and requires the `autoload` path. That is the hot path: one OPcache-cached config array, one key read, one autoload require.
