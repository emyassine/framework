...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div x-data="{
        ...
        translations: {
            searchPlaceholder: @js(__('imagery::search_placeholder')),
            allSets: @js(__('imagery::all_sets')),
            noResults: @js(__('imagery::no_results')),
            loading: @js(__('imagery::loading')),
            clear: @js(__('imagery::clear')),
            close: @js(__('imagery::close')),
            selectIcon: @js(__('imagery::select_icon')),
            cancel: @js(__('imagery::cancel')),
            iconsAvailable: @js(__('imagery::icons_available')),
        },
        endpoint: @js(route('imagery.icons')),
        ...
        async loadMore() {
            if (!this.has_more || this.isLoadingMore) return;
            this.currentPage++;
        ...
        handleScroll(event) {
            ...
            if ((el.scrollTop + el.clientHeight) >= (el.scrollHeight - 150)) {
                if (this.has_more && !this.isLoadingMore) {
                    this.loadMore();
                }
            }
        },
        ...
        async loadIconSvg(iconName, element) {
            ...
            try {
                const response = await fetch('/imagery/icon/' + encodeURIComponent(iconName));
                if (response.ok) {
                ...
                <template x-if="state">
                    <span class="fi-icon-picker-preview shrink-0" x-init="$watch('state', async (value) => {
                        if (value) {
                            ...
                            if (iconCache[value]) {
                                ...
                                const response = await fetch('/imagery/icon/' + encodeURIComponent(value));
                                if (response.ok) {
                                    const svg = await response.text();
                                    iconCache[value] = svg;
                                    $el.innerHTML = svg;
                    ...
                    if (state) {
                        ...
                        if (iconCache[state]) {
                            ...
                            fetch('/imagery/icon/' + encodeURIComponent(state))
                                .then(r => r.text())
                                .then(svg => {
                                    iconCache[state] = svg;
                                    $el.innerHTML = svg;
                    ...
                    <div class="px-6 pb-4">
                        <div class="flex gap-3">
                            ...
                            <div x-show="isSearchable" class="relative flex-1">
                                <div class="pointer-events-none absolute inset-y-0 flex items-center ps-3"
