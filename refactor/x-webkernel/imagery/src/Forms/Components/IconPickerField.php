...
namespace Webkernel\Imagery\Forms\Components;
...
{
    protected string $view = 'imagery::forms.components.icon-picker';
    ...
    protected function setUp(): void
    {
        ...
        $this->placeholder(fn (): string => __('imagery::placeholder'));
    }
    ...
    {
        $size = $this->evaluate($this->modalSize) ?? config('imagery.modal_size', '4xl');
    ...
    public function getAllowedSets(): ?array
    {
        ...
        $configSets = config('imagery.allowed_sets', []);
    ...
    public function getAvailableSets(): array
    {
        ...
        $all_sets = $manager->get_set_names();

        if ($allowedSets) {
            return array_values(array_intersect($all_sets, $allowedSets));
        }
