...
trait CanDumpTypography
{
    ...
    {
        $dir = TypographySystem::path(TypographySystem::DIR);
        /** @var array<string, list<string>> $by_pack */
        $by_pack = [];
        foreach (TypographySystem::catalog() as $slug => $meta) {
            ...
            $chunk = \str_replace('font-display: swap', 'font-display: optional', $chunk);
            $by_pack[$meta['pack']][] = $chunk;
        }
        foreach ($by_pack as $pack => $chunks) {
            \file_put_contents(
                TypographySystem::path(TypographySystem::fonts_css($pack)),
                $this->typography_css_header('WTS pack: '.$pack)."\n".\implode("\n", $chunks),
                \LOCK_EX,
            );
        }
    }
