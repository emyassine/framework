...
use Webkernel\Json\CanonicalJson\Canonicalizator;

/**
 * Fluent wrapper around Canonicalizator.
 *
 * Usage:
 *   webkernel_json_canonicalize($data)->to_string()
...
final class CanonicalJson implements \Stringable
{
    ...
    public function hash(string $strategy = 'sha256', bool $raw = false): string
    {
        ...
        if (!in_array($strategy, hash_algos(), true)) {
            throw new \InvalidArgumentException(
                sprintf('Unknown hash strategy "%s". Available: %s.', $strategy, implode(', ', self::supported_strategies()))
            );
        }
     ...
     * @return list<string>
     */
    public static function supported_strategies(): array
    {
    ...
    {
        if (!self::$engine instanceof Canonicalizator) {
