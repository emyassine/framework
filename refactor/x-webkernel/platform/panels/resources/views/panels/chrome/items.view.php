@foreach ($items as $item)
  @php
    $item_href = (string) ($item['href'] ?? '#');
    $item_label = (string) ($item['label'] ?? '');
    if (\str_starts_with($item_label, 'panel.')) {
      $item_label = lang($item_label);
    }
    $item_icon = (string) ($item['icon'] ?? 'package');
    $item_active = $panel_api->href_is_active($item_href);
  @endphp
  <li>
    <a href="{{ $item_href }}" class="wds-nav-link{{ $item_active ? ' wds-active' : '' }}"{!! $item_active ? ' aria-current="page"' : '' !!}>
      <span class="wds-nav-icon" aria-hidden="true">{!! icon($item_icon, 'wds-icon-svg') !!}</span>
      <span>{{ $item_label }}</span>
    </a>
  </li>
@endforeach
