...
namespace Webkernel\Platform\Colors;

/**
 * Palettes. CSS custom properties: `--color-{name}-{stop}`, `--primary-{stop}`.
 *
 * //> `--gray-*` aliases Zinc. Dark inverts gray only. Primary/danger/success/warning/info stay put.
 */
final class Color
{
    /** @var list<int> */
    public const STOPS = [50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950];
    ...
    /**
     * @param $name string
     * @param $shades array<int, string>
     *
     * @return string
     */
    public static function variables(string $name, array $shades): string
    {
        $css = '';
        foreach ($shades as $stop => $value) {
            $css .= '--'.$name.'-'.$stop.': '.$value.';'."\n";
        }
     ...
     * @return string
     */
    public static function alias(string $alias, string $palette): string
    {
        $css = '';
        foreach (self::STOPS as $stop) {
            $css .= '--'.$alias.'-'.$stop.': var(--color-'.$palette.'-'.$stop.');'."\n";
        }
    ...
    {
        $css = '';
        foreach (self::palettes() as $name => $shades) {
            $css .= self::variables('color-'.$name, $shades);
        }
        $css .= '--color-black: #000;'."\n";
        $css .= '--color-white: #fff;'."\n";
        $css .= self::alias('primary', 'blue');
        $css .= self::alias('gray', 'zinc');
        $css .= self::alias('danger', 'red');
        $css .= self::alias('success', 'green');
        $css .= self::alias('warning', 'amber');
        $css .= self::alias('info', 'blue');

        return $css;
    }

    /**
     * Dark mode remaps gray stops (50 <-> 950, …). Accent palettes do not invert.
     *
     * @return string
     */
    ...
    /**
     * Inline `--primary-*` from a hex. Empty hex yields empty string.
     *
     * @param $hex string
     *
     * @return string
     */
    public static function panel_inline(string $hex): string
    {
        ...
        $parts = [];
        foreach ($light as $stop => $pct) {
            $parts[] = '--primary-'.$stop.': color-mix(in oklch, '.$hex.' '.$pct.'%, white)';
        }
        $parts[] = '--primary-500: '.$hex;
        foreach ($dark as $stop => $pct) {
            $parts[] = '--primary-'.$stop.': color-mix(in oklch, '.$hex.' '.$pct.'%, black)';
        }
